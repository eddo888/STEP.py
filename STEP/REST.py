...
class EventProcessors(STEP):
	...
	def queue_enable(self, id: str) -> dict:
		try:
			response = super().post(f"{self.base}/{id}/queue/enable")
			return response
		except Exception as e:
			# Log the error and return an empty dictionary or handle as needed
			logger.error(f"An error occurred during the queue enable operation: {e}")
			return {}

	#________________________________________________________________________________________________
	@args.operation(help='disable the event queue associated with an event processor')
	def queue_disable(self, id: str) -> dict:
		try:
			response = super().post(f"{self.base}/{id}/queue/disable")
			return response
		except Exception as e:
			# Log the error and return an empty dictionary or handle as needed
